...
# Egg Economy ksqlDB seed data generator
# Same entities and events as egg-economy, backed by Kafka + ksqlDB.
#
# Usage: ./scripts/seed.sh [BASE_URL]
#   BASE_URL defaults to http://localhost:5090

BASE_URL="${1:-http://localhost:5090}"

echo "=== Egg Economy ksqlDB Seed Data ==="
...
# Get farm IDs — wait for ksqlDB materialization
sleep 3
...
# Wait for ksqlDB materialization
sleep 5
...
for hen_idx in "${!HEN_ARRAY[@]}"; do
    ...
    # Determine farm_id and coop_id based on hen index
    ...
    for day_offset in 0 1 2; do
        ...
        post_entity "lay_report/api" "$(jq -n --arg hid "$hen_id" --arg cid "$coop" --arg fid "$farm" --arg ts "$ts" --argjson eggs "$eggs" --arg q "${QUALITIES[$quality_idx]}" \
...
echo ""
echo "--- Phase 3: Waiting for Projections ---"
...
echo "=== Seed Complete ==="
echo ""
echo "Frontend URLs (via nginx on port 8090):"
echo "  Dashboard:   http://localhost:8090/dashboard/"
echo "  Homesteader: http://localhost:8090/homestead/"
echo "  Corporate:   http://localhost:8090/corporate/"

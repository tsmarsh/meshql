...
/**
 * 5-phase Kafka consumer that processes Debezium CDC messages from the Salesforce legacy
...
public class LegacyToCleanProcessor {
    ...
    private static final String TOPIC_PREFIX = "salesforce.public.";

    private static final Map<String, String> TOPIC_TO_API = Map.ofEntries(
            Map.entry(TOPIC_PREFIX + "farm__c", "/farm/api"),
            Map.entry(TOPIC_PREFIX + "coop__c", "/coop/api"),
            Map.entry(TOPIC_PREFIX + "hen__c", "/hen/api"),
            Map.entry(TOPIC_PREFIX + "storage_container__c", "/container/api"),
            Map.entry(TOPIC_PREFIX + "consumer_account__c", "/consumer/api"),
            Map.entry(TOPIC_PREFIX + "lay_report__c", "/lay_report/api"),
            Map.entry(TOPIC_PREFIX + "storage_deposit__c", "/storage_deposit/api"),
            Map.entry(TOPIC_PREFIX + "storage_withdrawal__c", "/storage_withdrawal/api"),
            Map.entry(TOPIC_PREFIX + "container_transfer__c", "/container_transfer/api"),
            Map.entry(TOPIC_PREFIX + "consumption_report__c", "/consumption_report/api")
    );
    ...
    public void start() {
        ...
        consumerThread = new Thread(this::consumeLoop, "salesforce-legacy-processor");
        consumerThread.start();

        logger.info("Started Salesforce legacy processor");
    }

    private Properties consumerProps() {
        Properties props = new Properties();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaBroker);
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "egg-economy-salesforce-processor");
    ...
    private void consumeLoop() {
        try {
            ...
            consumer.subscribe(List.of(
                    TOPIC_PREFIX + "farm__c",
                    TOPIC_PREFIX + "storage_container__c",
                    TOPIC_PREFIX + "consumer_account__c"
            ));
            drainTopic(consumer);
            consumer.commitSync();
            consumer.close();
            ...
            consumer = new KafkaConsumer<>(consumerProps());
            consumer.subscribe(List.of(TOPIC_PREFIX + "coop__c"));
            drainTopic(consumer);
            consumer.commitSync();
            consumer.close();
            ...
            consumer = new KafkaConsumer<>(consumerProps());
            consumer.subscribe(List.of(TOPIC_PREFIX + "hen__c"));
            drainTopic(consumer);
            consumer.commitSync();
            consumer.close();
            ...
            consumer = new KafkaConsumer<>(consumerProps());
            consumer.subscribe(List.of(
                    TOPIC_PREFIX + "lay_report__c",
                    TOPIC_PREFIX + "storage_deposit__c",
                    TOPIC_PREFIX + "storage_withdrawal__c",
                    TOPIC_PREFIX + "container_transfer__c",
                    TOPIC_PREFIX + "consumption_report__c"
            ));
            drainTopic(consumer);
            consumer.commitSync();
            consumer.close();
    ...
    private LegacyTransformer getTransformer(String topic) {
        if (topic.endsWith("farm__c")) return farmTransformer;
        if (topic.endsWith("coop__c")) return coopTransformer;
        if (topic.endsWith("hen__c")) return henTransformer;
        if (topic.endsWith("storage_container__c")) return containerTransformer;
        if (topic.endsWith("consumer_account__c")) return consumerTransformer;
        if (topic.endsWith("lay_report__c")) return layReportTransformer;
        if (topic.endsWith("storage_deposit__c")) return storageDepositTransformer;
        if (topic.endsWith("storage_withdrawal__c")) return storageWithdrawalTransformer;
        if (topic.endsWith("container_transfer__c")) return containerTransferTransformer;
        if (topic.endsWith("consumption_report__c")) return consumptionReportTransformer;
        return null;
    }

    private void registerIdMapping(String topic, JsonNode after, String meshqlId, ObjectNode cleanData) {
        if (topic.endsWith("farm__c")) {
            String legacyId = after.has("id") ? after.get("id").asText() : null;
            idResolver.registerFarm(legacyId, meshqlId);
        } else if (topic.endsWith("coop__c")) {
            String legacyId = after.has("id") ? after.get("id").asText() : null;
            idResolver.registerCoop(legacyId, meshqlId);
        } else if (topic.endsWith("hen__c")) {
            String legacyId = after.has("id") ? after.get("id").asText() : null;
            String coopId = cleanData.has("coop_id") ? cleanData.get("coop_id").asText() : null;
            idResolver.registerHen(legacyId, meshqlId, coopId);
        } else if (topic.endsWith("storage_container__c")) {
            String legacyId = after.has("id") ? after.get("id").asText() : null;
            idResolver.registerContainer(legacyId, meshqlId);
        } else if (topic.endsWith("consumer_account__c")) {
            String legacyId = after.has("id") ? after.get("id").asText() : null;
    ...
    private void routeEventToProjections(String topic, ObjectNode cleanData) {
        ...
        } else if (topic.endsWith("consumption_report__c")) {

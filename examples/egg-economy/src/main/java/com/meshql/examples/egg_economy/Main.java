...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        logger.info("Starting Egg Economy Application");

        // Read environment variables with defaults
        String actorsMongoUri = getEnv("ACTORS_MONGO_URI", "mongodb://localhost:27017/?replicaSet=rs0");
        String eventsMongoUri = getEnv("EVENTS_MONGO_URI", "mongodb://localhost:27017/?replicaSet=rs0");
        String projectionsMongoUri = getEnv("PROJECTIONS_MONGO_URI", "mongodb://localhost:27017/?replicaSet=rs0");
        ...
        logger.info("MongoDB URIs: actors={}, events={}, projections={}", actorsMongoUri, eventsMongoUri, projectionsMongoUri);
        logger.info("Kafka: broker={}", kafkaBroker);

        // --- Actor storage configs (Shard 1) ---
        MongoConfig farmDB = createMongoConfig(actorsMongoUri, prefix, env, "farm");
        MongoConfig coopDB = createMongoConfig(actorsMongoUri, prefix, env, "coop");
        MongoConfig henDB = createMongoConfig(actorsMongoUri, prefix, env, "hen");
        MongoConfig containerDB = createMongoConfig(actorsMongoUri, prefix, env, "container");
        MongoConfig consumerDB = createMongoConfig(actorsMongoUri, prefix, env, "consumer");

        // --- Event storage configs (Shard 2) ---
        MongoConfig layReportDB = createMongoConfig(eventsMongoUri, prefix, env, "lay_report");
        MongoConfig storageDepositDB = createMongoConfig(eventsMongoUri, prefix, env, "storage_deposit");
        MongoConfig storageWithdrawalDB = createMongoConfig(eventsMongoUri, prefix, env, "storage_withdrawal");
        MongoConfig containerTransferDB = createMongoConfig(eventsMongoUri, prefix, env, "container_transfer");
        MongoConfig consumptionReportDB = createMongoConfig(eventsMongoUri, prefix, env, "consumption_report");

        // --- Projection storage configs (Shard 3) ---
        MongoConfig containerInventoryDB = createMongoConfig(projectionsMongoUri, prefix, env, "container_inventory");
        MongoConfig henProductivityDB = createMongoConfig(projectionsMongoUri, prefix, env, "hen_productivity");
        ...
        // Start the event workflow processor
        String eventsDb = prefix + "_" + env;
        EventWorkflowProcessor processor = new EventWorkflowProcessor(
                kafkaBroker,
                eventsDb,
                platformUrl
        );
        ...
        logger.info("Event workflow processor started");

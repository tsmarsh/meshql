...
public abstract class ProjectionUpdater {
    ...
    /**
     * POST a new projection via REST, then discover its MeshQL ID via GraphQL.
     * MeshQL returns 201 without the generated ID, so we query by FK to find it.
     */
    protected String createProjection(String apiPath, ObjectNode data,
        ...
        try {
            ...
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                // Discover the ID via GraphQL query
    ...
    /**
     * Query GraphQL to find a projection's MeshQL ID by its FK query.
     */
        ...
        try {
            ...
            if (resp.statusCode() >= 200 && resp.statusCode() < 300) {
                ...
                if (results.isArray() && !results.isEmpty()) {
                    // Return the last entry (most recently created)
